...
#     # ============== model cfg =============
#     frags = ['20231215151901','frag5']#'frag5','frag1']#,'20231215151901']#'frag1',
...
#     # print("mask shape before pad",mask.shape)
#     mask = np.pad(mask, [(0, pad0), (0, pad1)], constant_values=0)
#     # print("mask shape",mask.shape)
#     mask = mask.astype('float32')
#     mask/=255 # Binarize
#     fragment_mask =fragment_mask/255
...
#                     for f in [['20231210132040','frag5']]:
#                         CFG.frags = f
#                         CFG.valid_id = '20231210132040'
#                         print(f"Validddddddddddddddd: {CFG.valid_id}")
...
#                         callbacks=[ModelCheckpoint(filename=f'f15_div2_l15-35_{fold}_fr_{enc}'+'{epoch}',dirpath=CFG.model_dir,monitor='train/total_loss',mode='min',save_top_k=CFG.epochs),
#                                     ],
#                         )
...
def read_image_mask(fragment_id, start_idx, in_chans):
    ...
    #     # scale factors for each axis: (height, width, depth)
    #     scale_factors = (1, 1, 30 / 60)
    #     images = zoom(images, scale_factors, order=1)  # order=1 → bilinear interpolation
...
valid_mask_gt = cv2.resize(valid_mask_gt, (valid_mask_gt.shape[1]//2, valid_mask_gt.shape[0]//2), interpolation=cv2.INTER_NEAREST)
