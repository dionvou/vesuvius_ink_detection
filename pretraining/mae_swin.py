...
class CFG:
    ...
    # pretraining/pretraining_scrolls
    # ============== fold =============
...
class TileDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = self.fourth_augment(image)     
            # ✅ Ensure shape matches CFG.size, CFG.size, CFG.valid_chans
        assert image.shape == (CFG.size, CFG.size, CFG.valid_chans), \
            f"Expected {(CFG.size, CFG.size, CFG.valid_chans)} but got {image.shape} for {self.tile_paths[idx]}"     
        if self.transform:
...
train_loader = DataLoader(train_dataset, batch_size=CFG.train_batch_size, shuffle=True, num_workers=32)
val_loader   = DataLoader(val_dataset, batch_size=CFG.valid_batch_size, shuffle=True, num_workers=1)
...
def visualize_reconstruction(original, reconstructed, sample_idx=0, num_frames=8,save_path='./results/reconstruction.png',epoch=0):
    ...
    if save_path is not None:
        os.makedirs(os.path.dirname(f'./results/reconstruction{epoch}.png'), exist_ok=True)
        plt.savefig(f'./results/reconstruction_{epoch}.png', dpi=300, bbox_inches='tight')
    plt.close(fig)



class MAEPretrain(pl.LightningModule):
    def __init__(self, lr=1e-5, mask_ratio=0.75, embed_dim=768, decoder_dim=512, decoder_layers=4):
        super().__init__()
        ...
        new_weight = weight.sum(dim=1, keepdim=True) #/ 3.0  # [128, 1, 2, 4, 4]
        ...
        self.unmasked_patches = int((1- self.mask_ratio)* self.N)
    ...
    def configure_optimizers(self):
        optimizer = AdamW([
            {'params': self.encoder.parameters(), 'lr': 1e-5, 'weight_decay': 0.05},
...
checkpoint_callback = ModelCheckpoint(
    ...
    filename="tiny_{epoch}",           # filename pattern
