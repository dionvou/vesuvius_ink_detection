...
def read_image_mask(fragment_id, start_idx, in_chans):
    ...
    # if (any(sub in fragment_id for sub in  ["202","left"])):
    #     idxs = range(start_idx, end_idx)
    for i in idxs:
    ...
    # images = np.stack([np.rot90(images[:, :, i]) for i in range(images.shape[2])], axis=2)
    ...
    # fragment_mask = np.rot90(fragment_mask).copy()
                        ...
                        if fragment_id =='s4':
                            if (y1,y2,x1,x2) not in windows_dict:
                                if not np.any(fragment_mask[a:a+ CFG.tile_size, b:b + CFG.tile_size]==0):
                                    # if not np.all(mask[a:a + CFG.tile_size, b:b + CFG.tile_size]<0.99):
                                        # if not np.any(fragment_mask[a:a+ CFG.tile_size, b:b + CFG.tile_size]==0):
                                        ...
                                        tile_mask[(tile_mask <1) & (tile_mask>0)] = 127 # mask
...
class CustomDataset(Dataset):
    ...
    def fourth_augment(self,image):
        ...
        cropping_num = random.randint(CFG.in_chans-4, CFG.in_chans)
...
# valid_mask_gt = np.rot90(valid_mask_gt).copy()
...
# fragments=['vals42']
# cfg_init(CFG)
for lr in [8e-5,2e-6,1e-4]:
    # for frags in [['frag5','frag1'],['frag5','left'],['frag1','left'],['frag6','frag1','left']]:

        CFG.lr=lr
        # CFG.frags=frags
        # CFG.train_batch_size=batch_size
        # CFG.in_chans = in_chans
        # CFG.start_idx = 32 - CFG.in_chans/2
        
        # print('in_chans: ',in_chans)
        # print('start_idx: ',CFG.start_idx)
        ...
        for fid in fragments:
            ...
            # pred_shape=valid_mask_gt.shape
            # if (any(sub in fragment_id for sub in ["frag", "rect","remaining",'s4','202'])):
            ...
            # # # DION
            # checkpoint = torch.load("outputs/vesuvius/pretraining_all/vesuvius-models/pseudo_['s4', 'frag1', 'frag5', '20231210132040']_0_fr_i3depoch=11.ckpt", map_location="cpu", weights_only=False)
            ...
            trainer = pl.Trainer(
                ...
                callbacks=[ModelCheckpoint(filename=f'pseudo_{CFG.frags}_{fold}_fr_{enc}'+'{epoch}',dirpath=CFG.model_dir,monitor='train/total_loss',mode='min',save_top_k=CFG.epochs),
                        ],
            )
