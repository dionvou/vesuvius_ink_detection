...
# for f in [['20240304161941','Frag5'],['20231210132040','Frag5','Frag1']]:  #'s4','omega',
#     for ratio in [2]:
#         for smooth in [0.25]:  #0.1,0.25
#             # CFG.valid_id = f[0]
#             # CFG.frags = f
#             # norm = True
#             max = True
...
#             run_slug=f'RESNET_valid={CFG.valid_id}_size={CFG.size}_lr={CFG.lr}_in_chans={CFG.in_chans}_norm={CFG.norm}_a={a}_max={max}_smooth={smooth}'

#             wandb_logger = WandbLogger(project="vesivus",name=run_slug)
#             model=RegressionPLModel(enc='resnet101',pred_shape=pred_shape,size=CFG.size,total_steps=len(train_loader), with_norm=CFG.norm, a = a,b = 1-a, smooth=smooth)
...
for f in [['20231210132040','Frag5'],['20231210132040','Frag5']]:  #'s4','omega',
    smooth = 0.25
    norm = False
    for ratio in [2]:
        CFG.valid_id = f[0]
        ...
        train_dataset = utils.VideoDataset(
            train_images, CFG, labels=train_masks, transform=utils.get_transforms(data='train', cfg=CFG), scale_factor=4)
        
        valid_dataset = utils.VideoDataset(
            valid_images, CFG,xyxys=valid_xyxys, labels=valid_masks, transform=utils.get_transforms(data='valid', cfg=CFG), scale_factor=4)
