...
class TTATransform:
    def __init__(self):
        self.transforms = [
            ...
            lambda x: torch.flip(x, dims=[-2, -1]), # HV flip
        ]
        self.inverse = [
            lambda x: x,
...
#         }
    def forward_tta(self, x):
        tta = TTATransform()
        preds = []
        for t, inv in zip(tta.transforms, tta.inverse):
            logits = self(t(x))
            preds.append(inv(logits))
    ...
    def validation_step(self, batch, batch_idx):
        ...
        logits = self.forward_tta(x) if self.use_tta else self(x)
        loss = self.loss_func(logits, y)
        ...
        for i, (x1, y1, x2, y2) in enumerate(xyxys):
            pred = F.interpolate(probs[i:i+1], size=(self.hparams.size, self.hparams.size), mode='bilinear')
            gt = F.interpolate(y[i:i+1], size=(self.hparams.size, self.hparams.size), mode='bilinear')


            self.mask_pred[y1:y2, x1:x2] += pred[0, 0].numpy()
            self.mask_gt[y1:y2, x1:x2] = gt[0, 0].numpy()
            self.mask_count[y1:y2, x1:x2] += 1


        self.log('val/total_loss', loss, prog_bar=True, sync_dist=True)
        return loss


    # =========================
    # EPOCH END (UNCHANGED METRICS)
    # =========================
    def on_validation_epoch_end(self):
        self.mask_pred /= np.maximum(self.mask_count, 1)
        pred_bin = (self.mask_pred > 0.5).astype(np.float32)
        gt = self.mask_gt.astype(np.float32)


        inter = np.logical_and(pred_bin, gt).sum()
        union = np.logical_or(pred_bin, gt).sum()
        iou = inter / (union + 1e-8)
        dice = 2 * inter / (pred_bin.sum() + gt.sum() + 1e-8)
    ...
    # =========================
    # PREDICT STEP (WITH TTA)
    # =========================
    def predict_step(self, batch, batch_idx):
        x, _, xyxys = batch
        logits = self.forward_tta(x) if self.use_tta else self(x)
        return torch.sigmoid(logits).cpu(), xyxys   
...
for f in [['20231210132040','Frag5']]:  #'s4','omega',
    smooth = 0.25
    ...
    for norm in [True,False]:
        ...
        # # CHECKPOINT
        # checkpoint = torch.load("outputs/resnet3d101/RESNET_['20231210132040', 'Frag5', 'Frag1']_valid=20231210132040_size=256_lr=2e-05_in_chans=30_norm=True_a=0.6_max=True_smooth=0.25_epoch=epoch=23.ckpt", map_location="cpu", weights_only=False)
